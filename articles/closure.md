...
外层函数调用结束就该调用`fn`了，调用`fn`实际上调用的就是`getNum`，所以此时`getNum`的活动对象会被创建并且入栈，由于`getNum`并没有自己的局部变量或者函数，所以该对象时一个控对象。在执行`++n`的时候就会沿着作用域链对象来查找变量`n`，于是找到了`numCompany`活动对象里的`n`将其自增，此时我们就可以得到2。
